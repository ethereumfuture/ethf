...
#if defined(NDEBUG)
#error "ETHF cannot be compiled without assertions."
#endif
...
{
    CAmount nSubsidy = 0;
	//CAmount nSlowSubsidy = 10 * COIN;
	int i; int c;

    // POW Year 0.5 + POS year 0.5-1
	if (nHeight == 0) { 
		//nSubsidy = 720000.00 * COIN;
		nSubsidy = 711000.00 * COIN;
	} else if (nHeight == 57602) { //for test of POS
		nSubsidy = 150000.00 * COIN;
    ...
    // POS next years
        ...
        nSubsidy = 10 * COIN;
		i=(int)floor(nHeight/345600);
		for (c=0; c<i; c++) {
			nSubsidy=nSubsidy-nSubsidy*9/100;
		}
    }
	//TODO - block value correction after 6 month here	
	return nSubsidy;
}
...
bool ProcessMessages(CNode* pfrom)
{
    ...
    while (!pfrom->fDisconnect && it != pfrom->vRecvMsg.end()) {
        ...
        if (fDebug)
            LogPrintf("ProcessMessages(message %u msgsz, %u bytes, complete:%s, peer=%d)\n",
                    msg.hdr.nMessageSize, msg.vRecv.size(),
                    msg.complete() ? "Y" : "N", pfrom->id);
